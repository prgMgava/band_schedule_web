...
export const TableFinances = () => {
  ...
  function createData(checkout: ICheckout) {
    return {
      acoes: [
        {
      ...
      type: (
        <>
          <div
            style={{
              transform: checkout.type == 1 ? "rotate(180deg)" : "",
              color: checkout.type == 1 ? "green" : "red",
            }}
          >
            {<ArrowDownward />}
          </div>
      ...
      value: Number(checkout.value).toLocaleString("pt-br", { style: "currency", currency: "BRL" }) || "--",
      date: (
        <>
  ...
  return (
    <Paper sx={{ width: "100%", overflow: "hidden", marginTop: "16px" }} className="finances-table">
            ...
            {rows.map(row => (
              <StyledTableRow key={uuid()} sx={{ ":hover": { backdropFilter: "contrast(0.5)" } }}>
                <StyledTableCell style={{ textAlign: "center" }}>
                  {row.acoes.map((item, index) => (
                    <span key={index} onClick={() => item.action(row)}>
                      {item.icon}
                    </span>
                  ))}
                </StyledTableCell>
                ...
                <StyledTableCell title={row.description}>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-slide-description">Esta ação não poderá ser desfeita</DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloseModal}>Cancelar</Button>
          <Button onClick={deleteFinance} color="error">
            Excluir
          </Button>
        </DialogActions>
      </Dialog>
