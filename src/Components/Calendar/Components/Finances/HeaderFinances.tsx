...
export const HeaderFinances = () => {
  ...
  const getSums = () => {
    const sumEntrance: any = checkouts
      ?.filter(checkout => checkout.type == 1)
      .reduce((a, b) => {
        return a + Number(b.value)
      }, 0)

    setEntrance(sumEntrance?.toLocaleString("pt-br", { style: "currency", currency: "BRL" }))
    const sumOutput: any = checkouts
      ?.filter(checkout => checkout.type == 2)
      .reduce((a, b) => {
        return a + Number(b.value)
      }, 0)
  ...
  return (
    <Card style={{ padding: "16px", margin: "0px auto", position: "relative" }}>
      ...
      >
        <Box
          style={{
            borderRight: !mobile ? "solid 2px #e1e1e1" : "",
            borderBottom: mobile ? "solid 2px #e1e1e1" : "",
        ...
        >
          ...
          <Typography variant="subtitle2" gutterBottom color="#707070">
